...
// 注意： 被拷贝对象、拷贝的对象的属性列，必须一一对应！！！
//  @Description: 对象属性拷贝
//  @param dst 拷贝到指定对象
//  @param src 当前要拷贝的对象 例如：err = SimpleCopyProperties(req, spuDb)，把req的属性拷贝到spuDb，req值为空不拷贝，spuDb的匿名对象属性无法拷贝
//  @return err
//func SimpleCopyProperties(src, dst interface{}) (err error) {
//	// 防止意外panic
//	defer func() {
//		if e := recover(); e != nil {
...
//		//propertyValueDst := dstValue.FieldByName(property.Name)
//		//fmt.Println(propertyValue)
//		//fmt.Println(isBlank(propertyValue))
//		//fmt.Println(propertyValue.IsValid())
//		//fmt.Println(propertyValueDst)
//
//		// 无效，说明src没有这个属性 || 属性同名但类型不同
//		if !propertyValue.IsValid() || property.Type != propertyValue.Type() || isBlank(propertyValue) {
//			//fmt.Println(propertySrc.Name)
//			//fmt.Println(propertySrc.Type.String())
//			//fmt.Println(property.Name)
//			//fmt.Println(property.Type.String())
//
//			if propertyValue.IsValid() && !isBlank(propertyValue) {
//				// 日期处理========》将json结构体中的 自定义日期类型 commons.Time 转换为 time.Time类型拷贝到 model.结构体种
//				if propertySrc.Type.String() == "commons.Time" && property.Type.String() == "time.Time" {
//					// 获取当前属性值
...
//					// 通过反射设置到目标对应的属性种
//					dstValue.Field(i).Set(tValue)
//				} else if propertySrc.Type.String() == "time.Time" && property.Type.String() == "string" {
...
//			// boolean类型处理=========》
//			if propertyValue.IsValid() && isBlank(propertyValue) {
//				if propertySrc.Type.String() == "bool" && property.Type.String() == "bool" {
